...
**把电商和自媒体的平台导出文件，变成干净的仪表盘、客户分析和自然语言查询 —— 自托管，零爬虫。**
...
你在有赞、京东、天猫上卖货，也在公众号、小红书、知乎上做内容。每个平台都能导出数据，但导出来的是几十个列名各不相同的 Excel 表格 —— 不同平台对"客户"的定义不一样、有些指标是累计值、同一笔订单可能有多行记录需要去重。

通用 BI 工具（如 Tableau、Metabase）不会替你处理这些问题。它们只是在你给的表结构上画图，至于图上画的东西对不对，全靠你自己。

**OmniPanel 替你把这些脏活干了。** 上传你的导出文件，它会自动：

- 🏷️ **识别平台** —— 根据列名指纹判断文件来自哪个平台，不需要你手动选
- 🔄 **归一化** —— 把不同平台的订单，统一成同一套字段（客户、金额、时间、地区……）
- 📊 **给你分析** —— 仪表盘、客户画像、队列留存、跨平台客户合并、SQL 查询台、中文问数据

数据来源全部是**平台官方提供的导出功能** —— 不涉及爬虫，不走灰色地带。你有这些数据的合法所有权，OmniPanel 只是把它们变成你真正能用的样子。

### 使用流程，三分钟上手

```
你有导出文件 ──→ 上传 ──→ OmniPanel 自动识别、清洗、入库 ──→ 得到仪表盘和分析
```

1. **导出** —— 从有赞 / 京东 / 天猫后台导出订单表格，或从公众号 / 小红书 / 知乎导出内容数据
2. **上传** —— 拖进 OmniPanel，它会自动识别平台来源，归一化后存入数据库，同时保留原始行方便核对
3. **分析** —— 打开仪表盘看数据，在 SQL 查询台写 SQL，或者直接用中文提问（比如"上个月广东地区复购率是多少"）
...
OmniPanel 只摄入你自己合法拥有的官方导出数据 —— 结构稳定、格式规范、不受前端改版影响。把精力花在通用 BI 工具不会替你做的那一层正确性工作上，而不是花在和平台对抗反爬机制上。
...
| 项目 | 数据来源 | 实际交付 |
|---|---|---|
| **OmniPanel**（本仓库） | 官方导出（有赞/京东/天猫，公众号/小红书/知乎） | 自托管应用：仪表盘、队列留存、跨平台客户识别、SQL 查询台、中文问数据 |
