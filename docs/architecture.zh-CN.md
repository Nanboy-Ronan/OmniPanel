...
![OmniPanel 架构图](images/architecture.zh-CN.png)

下面的图把同一张图拆得更细：先是核心请求流程，然后是后端各模块的细节，最后是两个涉及外部方的流程（企业微信单点登录、中文问数据服务商）。
...
FastAPI 后端（`app/`）内部组织为：

```
┌──────────────────────────────────────────────┐
│  鉴权 —— JWT + 企业微信（WeCom）单点登录，   │
│  基于角色的访问控制（viewer/analyst/admin）  │
└──────────────────────────────────────────────┘
┌─────────────────────────────────────┐
│  ETL —— 识别平台 → 归一化 → 入库    │
...
└────────────────────────────────────────┘
┌─────────────────────────────────────────────┐
│  SQL 查询台 + 中文问数据 —— 仅允许 SELECT/  │
│  WITH、自动加 LIMIT、只读事务、审计日志     │
└─────────────────────────────────────────────┘
┌────────────────────────────────────┐
│  后台任务（leader 选举）——         │
│  微信指标自动同步、数据库月度备份  │
...
可选 —— 配置任意服务商的 API Key 后启用：

```
┌───────────────────────────────────────────┐
│  中文问数据服务商：Anthropic / OpenAI /   │
│  MiniMax / DeepSeek / 月之暗面 / 智谱     │
│  （只负责生成 SQL 文本；生成结果会经过和  │
│  人工输入完全一样的 SQL 查询台安全防护）  │
└───────────────────────────────────────────┘
```
